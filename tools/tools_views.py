...
def merge_frames_dir(source_dir, target_dir):
  # Move every file from source_dir into target_dir without touching what is
  # already there. Missing subdirectories are created on the fly. Returns
  # {old_relative_name : new_relative_name} for every file that had to be
  # renamed because a file of that name existed already.
  renamed = {}
  ...
  for source_path in sorted(source_dir.rglob('*')):
    ...
    if target_path.exists():
      ...
      while target_path.exists():
        ...
        counter += 1
      renamed[rel_path.as_posix()] = (
        target_path.relative_to(target_dir).as_posix())
    shutil.move(str(source_path), str(target_path))
  shutil.rmtree(source_dir, ignore_errors = True)
  return(renamed)

def merge_coded_dir(source_dir, target_dir, renamed):
  # Same as merge_frames_dir(), but the layout is <resolution>/<bucket>/
  # <file>.cod. The rename map of the frames is applied here as well, so that
  # image and coded file keep the same base name.
  target_dir.mkdir(parents = True, exist_ok = True)
  for source_path in sorted(source_dir.rglob('*')):
    if not source_path.is_file():
      continue
    rel_path = source_path.relative_to(source_dir)
    if renamed and len(rel_path.parts) > 1:
      frames_key = Path(*rel_path.parts[1:]).with_suffix('.bmp').as_posix()
      if frames_key in renamed:
        rel_path = (Path(rel_path.parts[0])
          / Path(renamed[frames_key]).with_suffix('.cod'))
    target_path = target_dir / rel_path
    target_path.parent.mkdir(parents = True, exist_ok = True)
    shutil.move(str(source_path), str(target_path))
  shutil.rmtree(source_dir, ignore_errors = True)

def patch_db_dat(data_file, renamed):
  # Rewrite the name column of the export file for all frames that were
  # renamed while merging, so that the DB rows still point to real files.
  name_index = trainframe_imex.index('name')
  patched_file = data_file.with_name(data_file.name + '.patched')
  with data_file.open('r', encoding = 'utf-8') as f_in:
    with patched_file.open('w', encoding = 'utf-8') as f_out:
      for line in f_in:
        fields = line.rstrip('\n').split('\t')
        if len(fields) > name_index and fields[name_index] in renamed:
          fields[name_index] = renamed[fields[name_index]]
        f_out.write('\t'.join(fields) + '\n')
  patched_file.replace(data_file)
...
class process_restore(myTemplateView):  
  ...
  def post(self, request, *args, **kwargs):
    ...
    if self.job_type == 'restore':
    ...
    elif self.job_type == 'addition':
      school_line = school.objects.get(id = int(self.post_dict['school_nr']))
      this_school_path = SCHOOLSPATH / f'model{school_line.id}'
      frames_path = this_school_path / 'frames'
      coded_path = this_school_path / 'coded'
      renamed = {}
      if (unpack_dir / 'frames').exists():
        renamed = merge_frames_dir(unpack_dir / 'frames', frames_path)
      if (unpack_dir / 'coded').exists():
        merge_coded_dir(unpack_dir / 'coded', coded_path, renamed)
      if renamed and (unpack_dir / 'db.dat').exists():
        patch_db_dat(unpack_dir / 'db.dat', renamed)
      do_load_db = True
    elif self.job_type == 'import':
      ...
      do_load_db = check_frames
    if self.job_type in ('addition', 'import', ):
      data_file = unpack_dir / 'db.dat'
      if do_load_db and data_file.exists():
        ...
        # The export writes school = 0, so only the freshly loaded rows are
        # touched here. Nothing existing is deleted or modified:
    ...
    if self.job_type == 'restore':
      ...
      context.update({
        'code' : 10,
        'message' : 'Done injecting school. You can continue...',
      })
    return self.render_to_response(context)
