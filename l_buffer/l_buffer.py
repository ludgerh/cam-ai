...
class l_buffer():
  def __init__(self, struct, m_proc = True, q_len = 1, block_put = True, 
      block_get = True, put_timeout = None, get_timeout = None, call = None, 
    ...
    if self.m_proc:
      ...
      self.put_storage = [deepcopy(PUT_STORAGE_ITEM) for _ in struct]
      self.get_storage = [deepcopy(GET_STORAGE_ITEM) for _ in struct]
      self.shm_dict = {}
  ...
  async def put_to_shelf(self, data): 
    if self.block_put:
      while self.shelf or self.shelf is None:
  ...
  async def put(self, data):
    ...
    while len(data) > len(self.struct):
      self.struct = self.struct + self.struct[-1]
      if self.m_proc: 
  ...
  async def get(self):
    ...
    while not self.shelf and self.shelf is not None:
      await asyncio.sleep(self.brake_time)
    data_in = self.shelf  
    self.new_data = False
    if self.block_put: 
      self.shelf = False
    if self.debug:
        print('From Shelf:', data_in)
    if data_in == 'stop' or data_in is None:
      result = data_in
    else:  
      result = [] 
      if self.q_len is not None:
        while len(data_in) > len(self.struct):
          self.struct = self.struct + self.struct[-1]
          if self.m_proc:
            self.get_storage.append(deepcopy(GET_STORAGE_ITEM))
      for item in data_in:  
        i, data = item
        if not self.m_proc or self.struct[i] == 'O':
          result.append(data)
        else:
          storage = self.get_storage[i]
          if 'len' in data:
  ...
  async def empty(self): #might cause problems if block_put is False
    if self.q_len is None: 
      return(self.shelf is not False)
    else:
      return(await asyncio.to_thread(self.data_queue.empty) and self.shelf is not False)  
    ...
    if self.mode == 'P':
      ...
      if self.q_len is not None:
        ...
        while not self.data_queue.empty():
          await asyncio.sleep(self.brake_time) 
      await self.put_to_shelf('stop') 
      if self.m_proc:
        for item in self.shm_dict.values():
          item.close()
          item.unlink()
    self.do_run = False
